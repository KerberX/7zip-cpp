...
namespace SevenZip
{
	namespace intl
	{
		class ScannerCallback: public PathScanner::Callback
		{
			...
			public:
				ScannerCallback(bool recursive, bool onlyFirst = false)
					:m_recursive(recursive), m_onlyFirst(onlyFirst)
				{
				}
		...
		TString FileSys::ExtractRelativePath(const TString& basePath, const TString& fullPath)
		{
			...
			TString sOut = fullPath.substr(basePath.size(), fullPath.size() - basePath.size());
			if (!sOut.empty() && sOut.front() == _T('\\'))
			{
				sOut.erase(0, 1);
			}
			return sOut;
		...
		CComPtr<IStream> FileSys::OpenFileToRW(const TString& filePath)
		{
			CComPtr<IStream> fileStream;
			if (FAILED(SHCreateStreamOnFileEx(filePath.c_str(), (FileExists(filePath) ? 0 : STGM_CREATE)|STGM_READWRITE, FILE_ATTRIBUTE_NORMAL, FALSE, NULL, &fileStream)))
			{
				return NULL;
			}
