...
namespace SevenZip
{
	namespace intl
	{
		...
		STDMETHODIMP ArchiveExtractCallback::GetStream(UInt32 index, ISequentialOutStream** outStream, Int32 askExtractMode)
		{
			...
			if (canContinue)
			{
				if (m_FinalPaths.empty())
				{
					...
					m_AbsolutePath = FileSys::AppendPath(m_Directory, m_RelativePath);
				}
				...
				if (m_IsDirectory)
				{
					...
					FileSys::CreateDirectoryTree(m_AbsolutePath);
					*outStream = NULL;
					return S_OK;
				}

				FileSys::CreateDirectoryTree(FileSys::GetPath(m_AbsolutePath));

				CComPtr<IStream> fileStream = FileSys::OpenFileToWrite(m_AbsolutePath);
				if (fileStream == NULL)
				{
		...
		STDMETHODIMP ArchiveExtractCallback::SetOperationResult(Int32 operationResult)
		{
			...
			{
				HANDLE fileHandle = CreateFile(m_AbsolutePath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
				if (fileHandle != INVALID_HANDLE_VALUE)
				{
					SetFileTime(fileHandle, NULL, NULL, &m_ModificationTime);
